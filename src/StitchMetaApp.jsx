...
export default function StitchMetaApp({ accessToken = '', embedded = false, onLogout }) {
  ...
  useEffect(() => {
    ...
    setSession(prev => ({ ...prev, token: accessToken }));
  }, [accessToken]);
  ...
  const handleLogout = useCallback(() => {
    setSession({ loggedIn: false, token: '', appId: APP_ID });
    setAccounts([]);
    setSelectedAccount(null);
    setInsights(null);
    setLogs([]);
  ...
  useEffect(() => {
    if (!embedded || !accessToken || session.loggedIn || isConnecting || hasAutoConnected.current) {
      return;
    }
  ...
  if (!session.loggedIn && embedded) {
    return (
      <div className="app-wrapper" style={{ justifyContent: 'center', alignItems: 'center' }}>
        <style>{STYLES}</style>
        <div className="glass-panel content-fade-in" style={{ padding: '2rem', maxWidth: '520px', margin: '2rem', textAlign: 'center' }}>
          <div style={{ marginBottom: '1rem' }}>
            <div style={{ width: '64px', height: '64px', background: 'var(--logo-bg)', borderRadius: '16px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1rem', boxShadow: '0 10px 15px -3px rgba(255, 93, 0, 0.4)' }}>
              <img src={LOGO_URL} alt="Purge Digital logo" style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: '16px' }} />
            </div>
            <h2 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>Meta Risk Access</h2>
            <p className="text-small" style={{ textTransform: 'none' }}>
              {accessToken ? 'Connecting to Meta with your system user token.' : 'Add your system user token on the Purge CRM login screen to enable Meta Risk.'}
            </p>
          </div>
          {authError && (
            <div style={{ background: 'rgba(255, 93, 0, 0.1)', border: '1px solid rgba(255, 93, 0, 0.2)', padding: '1rem', borderRadius: '0.75rem', display: 'flex', gap: '0.5rem', color: '#ff8c42', fontSize: '0.875rem', justifyContent: 'center' }}>
              <AlertTriangle size={16} /> {authError}
            </div>
          )}
          {isConnecting && (
            <div style={{ marginTop: '1rem', display: 'inline-flex', alignItems: 'center', gap: '0.5rem', color: '#fff' }}>
              <Loader2 size={18} className="animate-spin-always" /> Initializing...
            </div>
          )}
        </div>
      </div>
    ...
    <div className="app-wrapper">
      ...
      <nav className="nav-bar">
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          ...
          <LogOut size={20} style={{ cursor: 'pointer', color: '#94a3b8' }} onClick={handleLogout} />
        </div>
      </nav>
